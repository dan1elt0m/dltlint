...
def _validate_libraries(doc: Dict[str, Any], root: str) -> List[Finding]:
    ...
    for i, item in enumerate(libs):
        ...
        if len(present) == 0:
            f.append(Finding(code="DLT421", message="library should specify one of: notebook|file|jar|whl|maven|pypi", path=loc, severity=Severity.WARNING))
            continue
        if len(present) > 1:
            f.append(Finding(code="DLT423", message=f"library must specify exactly one kind, found {present}", path=loc, severity=Severity.WARNING))

        kind = present[0]
        def _require_path(obj, where: str) -> None:
            if isinstance(obj, str):
                return
            if not isinstance(obj, dict) or not isinstance(obj.get("path"), str):
                f.append(Finding(code="DLT422", message="library requires a string or an object with 'path'", path=where))
        ...
        if kind in ("notebook", "file"):
            ...
            if not isinstance(o, dict) or not isinstance(o.get("coordinates"), str):
                f.append(Finding(code="DLT425", message="maven requires object with 'coordinates' (e.g., group:artifact:version)", path=loc))
            else:
                if "exclusions" in o and (not isinstance(o["exclusions"], list) or not all(isinstance(x, str) for x in o["exclusions"])):
                    f.append(Finding(code="DLT425", message="maven.exclusions must be a list of strings", path=f"{loc}.maven.exclusions"))
            ...
            if not isinstance(o, dict) or not isinstance(o.get("package"), str):
                f.append(Finding(code="DLT426", message="pypi requires object with 'package' (e.g., 'duckdb==1.0.0')", path=loc))
            else:
                if "repo" in o and not isinstance(o["repo"], str):
...
def _validate_notifications(doc: Dict[str, Any], root: str) -> List[Finding]:
    ...
    for i, n in enumerate(notifs):
        ...
        if not (isinstance(recipients, list) and recipients and all(isinstance(x, str) for x in recipients)):
            f.append(Finding(code="DLT450", message="notification.email_recipients must be a non-empty list of strings", path=loc))
...
def _validate_clusters(doc: Dict[str, Any], root: str) -> List[Finding]:
    ...
    for i, cl in enumerate(clusters):
        ...
        if forbidden:
            f.append(Finding(code="DLT431", message="These cluster fields are managed by Lakeflow and must not be set: " + ", ".join(sorted(forbidden)), path=loc))
        ...
        if as_ is not None and not isinstance(as_, dict):
            f.append(Finding(code="DLT462", message="autoscale must be an object with 'min_workers' and 'max_workers'", path=f"{loc}.autoscale"))
        elif isinstance(as_, dict):
            mw = as_.get("min_workers")
            xw = as_.get("max_workers")
            if not (isinstance(mw, int) and isinstance(xw, int)):
                f.append(Finding(code="DLT463", message="autoscale.min_workers and autoscale.max_workers must be integers", path=f"{loc}.autoscale"))
            else:
                if mw < 0 or xw < 0:
                    f.append(Finding(code="DLT464", message="autoscale min/max workers must be >= 0", path=f"{loc}.autoscale"))
                if mw > xw:
                    f.append(Finding(code="DLT465", message="autoscale.min_workers must be <= autoscale.max_workers", path=f"{loc}.autoscale"))
        if isinstance(nw, int) and isinstance(as_, dict):
            f.append(Finding(code="DLT466", message="Specify either 'num_workers' or 'autoscale', not both", path=loc, severity=Severity.WARNING))
        ...
        for mapfld in ("spark_conf", "custom_tags"):
            m = cl.get(mapfld)
            if m is not None:
                if not isinstance(m, dict) or not all(isinstance(k, str) for k in m.keys()) or not all(isinstance(v, str) for v in m.values()):
                    f.append(Finding(code="DLT468", message=f"{mapfld} must be a mapping of string->string", path=f"{loc}.{mapfld}"))
    return f
...
def _lint_schema(doc: Dict[str, Any], known_fields: Dict[str, Any], *, root: str) -> List[Finding]:
    ...
    for k in doc.keys():
        if k not in known_fields:
            f.append(Finding(code="DLT010", message=f"Unknown top-level field '{k}'", path=f"{root}.{k}", severity=Severity.WARNING))

    for k, expected in known_fields.items():
        if k in doc:
            v = doc[k]
            if expected is str and not isinstance(v, str):
                f.append(Finding(code="DLT100", message=f"Field '{k}' must be a string, got {_type_name(v)}", path=f"{root}.{k}"))
            elif expected is bool and not isinstance(v, bool):
                f.append(Finding(code="DLT101", message=f"Field '{k}' must be a boolean, got {_type_name(v)}", path=f"{root}.{k}"))
            elif expected is int and not isinstance(v, int):
                f.append(Finding(code="DLT102", message=f"Field '{k}' must be an integer, got {_type_name(v)}", path=f"{root}.{k}"))
            elif expected is list and not isinstance(v, list):
                f.append(Finding(code="DLT103", message=f"Field '{k}' must be a list/array, got {_type_name(v)}", path=f"{root}.{k}"))
            elif expected is dict and not isinstance(v, dict):
                f.append(Finding(code="DLT104", message=f"Field '{k}' must be a mapping/object, got {_type_name(v)}", path=f"{root}.{k}"))

    if "channel" in doc and isinstance(doc["channel"], str):
        if doc["channel"] not in CHANNEL_VALUES:
            f.append(Finding(code="DLT200", message=f"channel must be one of {sorted(CHANNEL_VALUES)}", path=f"{root}.channel"))

    if "edition" in doc and isinstance(doc["edition"], str):
        if doc["edition"] not in EDITION_VALUES:
            f.append(Finding(code="DLT201", message=f"edition must be one of {sorted(EDITION_VALUES)}", path=f"{root}.edition"))

    if "pipelines.trigger.interval" in doc and isinstance(doc["pipelines.trigger.interval"], str):
        if not TRIGGER_INTERVAL_RE.match(doc["pipelines.trigger.interval"]):
            f.append(Finding(code="DLT202", message="pipelines.trigger.interval must be like '10 minutes' | '1 hour' | '30 seconds'", path=f"{root}.pipelines.trigger.interval"))
    ...
    if "trigger" in doc:
        if not isinstance(doc["trigger"], dict):
            ...
            if isinstance(ti, str) and not TRIGGER_INTERVAL_RE.match(ti):
                f.append(Finding(code="DLT202", message="trigger.interval must be like '10 minutes' | '1 hour' | '30 seconds'", path=f"{root}.trigger.interval"))

    has_modern = ("catalog" in doc) or ("schema" in doc)
    has_legacy = ("target" in doc) or ("storage" in doc)
    if has_modern and has_legacy:
        f.append(Finding(code="DLT300", message="Use either modern (catalog/schema) or legacy (target/storage) publishing, not both", path=root))

    if "name" not in doc:
        f.append(Finding(code="DLT400", message="Missing recommended field 'name'", path=root, severity=Severity.WARNING))

    for key in ("pipelines.maxFlowRetryAttempts", "pipelines.numUpdateRetryAttempts", "maxFlowRetryAttempts", "numUpdateRetryAttempts"):
    ...
    if isinstance(doc.get("configuration"), dict):
        for ck, cv in doc["configuration"].items():
            if not isinstance(ck, str):
                f.append(Finding(code="DLT410", message="configuration keys must be strings", path=f"{root}.configuration"))
                break
            if not isinstance(cv, (str, int, float, bool)):
                f.append(Finding(code="DLT411", message=f"configuration value for '{ck}' should be a scalar (string/number/bool)", path=f"{root}.configuration.{ck}", severity=Severity.WARNING))
...
def lint_pipeline(doc: Any, *, root: str = "$") -> List[Finding]:
    ...
    if isinstance(resources, dict) and isinstance(resources.get("pipelines"), dict):
        ...
        for pid, pobj in pipelines.items():
            if not isinstance(pobj, dict):
                findings.append(Finding(code="DLT002", message=f"Pipeline '{pid}' must be an object", path=f"{root}.resources.pipelines.{pid}"))
                continue
            ...
            if isinstance(res, dict) and isinstance(res.get("pipelines"), dict):
                for pid, pobj in res["pipelines"].items():
                    ...
                    for need in cfg.require:
                        if need not in pobj:
                            findings.append(Finding(code="DLT400", message=f"Missing required field '{need}'", path=f"{path}.resources.pipelines.{pid}", severity=Severity.ERROR))
            else:
                for need in cfg.require:
                    if need not in doc:
                        findings.append(Finding(code="DLT400", message=f"Missing required field '{need}'", path=str(path), severity=Severity.ERROR))
